...
module.exports = {
  ...
  getOfficers: (callBack) => {
    pool.query(`SELECT * FROM gramaniladari`, [], (error, results, fields) => {
      if (error) {
        return callBack(error);
      }
      return callBack(null, results);
  ...
  getOfficerGramaIdByOfficerID: (grmaniladari_officer_id, callBack) => {
    pool.query(
      `SELECT gramaniladari_division_id FROM gramaniladari WHERE grmaniladari_officer_id=?`,
      [grmaniladari_officer_id],
      (error, results, fields) => {
        if (error) {
          return callBack(error);
